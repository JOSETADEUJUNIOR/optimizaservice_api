...
class ChamadoDAO extends Conexao
{
    ...
    {
        $sql = $this->conexao->prepare(ChamadoSQL::AbrirChamadoSQL($vo->getSchema()));
        $i = 1;
        $dataAbertura = $vo->getDataAbertura();
        $horaAbertura = $vo->getHora_abertura();
        // Use um espaço em branco entre a data e a hora
        $dataHoraAbertura = $dataAbertura . " " . $horaAbertura;

        $sql->bindValue($i++, $dataHoraAbertura);
