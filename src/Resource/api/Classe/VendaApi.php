...
class VendaApi extends ApiRequest
{
    ...
    {
        return (new VendaController)->RetornarVendaController($this->params['tenant_id']);
    }
    public function DetalharVenda()
    {
        if (Util::AuthenticationTokenAccess()) {
            $vo = new VendaVO;
            $vo->setId($this->params['id_venda']);
            $vo->setSchema($this->params['tenant_id']);
           
            return (new VendaController)->DetalharVendaController($vo);
        } else {
    ...
    public function DetalharItensVenda()
    {
        if (Util::AuthenticationTokenAccess()) {
            $vo = new VendaVO;
            $vo->setId($this->params['id_venda']);
            $vo->setSchema($this->params['tenant_id']);
            return (new VendaController)->DetalharItensVendaController($vo);
        } else {
            return NAO_AUTORIZADO;
        }
    }
    ...
    public function CadastrarVenda()
    {
        ...
        $vo = new VendaVO;
        $vo->setCliID($this->params['cliente']);
        $vo->setDtVenda($this->params['dataVenda']);
        $vo->setDesconto($this->params['desconto']);
        $vo->setValorEntrega($this->params['valorEntrega']);
        $vo->setStatus($this->params['status']);
        $vo->setSchema($this->params['tenant_id']);
        return (new VendaController)->CadastrarVendaController($vo, $produtos);
    }

    public function CadastrarVendaPDV()
    {
        $produtos = $this->params['produtos'];
        $vo = new VendaVO;
        $vo->setCliID($this->params['cliente']);
    ...
    {
        $produtos = $this->params['produtos'];
        $itensExcluidos = $this->params['itensExcluidos'];
        $vo = new VendaVO;
        $vo->setID($this->params['id_venda']);
        $vo->setCliID($this->params['cliente']);
        $vo->setDtVenda($this->params['dataVenda']);
       ...
       // return $itensExcluidos;
        return (new VendaController)->EditarVendaController($vo, $produtos, $itensExcluidos);
    }


    public function FiltrarVendaDetalhada()
    {
        return (new VendaController)->FiltrarVendaDetalhadaController($this->params['tenant_id'], $this->params['VendaID']);
    }
